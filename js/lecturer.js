auth.onAuthStateChanged((user) => {
  ...
  firebase.database().ref('userDetails/' + emailid).once('value')
    .then((snapshot) => {
      ...
      if (!data || (data.Role || '').toString().toLowerCase() !== 'lecturer') {
        window.location.href = 'index.html'
        return
      }

      document.getElementById('lbusername').innerText = user.email
      document.getElementById('lbLecturerName').innerText = `${data.FirstName || ''} ${data.LastName || ''}`.trim()
      document.getElementById('lbLecturerStatus').innerText = data.Status || 'Unknown'

      firebase.database().ref('courses').once('value', (coursesSnapshot) => {
        let totalCourses = 0
        coursesSnapshot.forEach(() => totalCourses++)
        document.getElementById('lbLecturerCourses').innerText = totalCourses
      })
      ...
      firebase.database().ref('userDetails').once('value', (usersSnapshot) => {
        ...
        document.getElementById('lbLecturerStudents').innerText = activeStudents
      })
    })
})
